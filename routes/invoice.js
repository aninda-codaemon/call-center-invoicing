...
router.get("/test-dispatch", async (req, res) => {
  ...
  try {
    ...
    return res
      .status(200)
      .json({ errors: [], data: { msg: "", request: dispatchObject } });
  } catch (error) {
    console.log(error);
  }
});
  ...
  distance.matrix(origins, destinations, function (err, distances) {
    if (!err) {
      ...
      if (distances.rows[0]["elements"][0].status === "OK") {
        let tmiles = distances.rows[0]["elements"][0].distance.text.split(
          " "
        )[0];
        ...
        return res
          .status(400)
          .json({ errors: [{ msg: "Distance calculation error" }] });
      }
    } else {
      return res
        .status(400)
...
router.post(
  ...
  [
    authMiddleware,
    [
      check("ozip", "Please enter a valid origin zipcode")
        .not()
        .isEmpty(),
      check("servicetype", "Please enter service type")
        .not()
        .isEmpty(),
      check("lat", "Please enter origin latitude")
        .not()
        .isEmpty(),
      check("lng", "Please enter origin longitude")
        .not()
        .isEmpty()
    ]
  ],
  async (req, res) => {
    ...
    const total_distance = parseFloat(req.body.tmiles) || 0.0;
    const destination_zipcode = req.body.dzip ? req.body.dzip.toString() : "";
    const service_type = req.body.servicetype.toString();
    const additional_charges = parseFloat(req.body.addlcharges) || 0.0;
    ...
    if (msa.error) {
      ...
      const getLastHourCtmCallData = async function () {
        const ctm_flag = "search_activities";
        const last_hr_ctm_data = await callCtmAuthApi(ctm_flag);
        if (last_hr_ctm_data == true) {
          // Check  current call is Google Addword or not
          let is_current_call_addword = await InvoiceModel.checkCurrentCallAddword(
            mobile_number
          );
          if (is_current_call_addword.result.length > 0) {
            ...
            if (call_payment_status.result.length > 0) {
              ...
              call_payment_status.result.forEach(function (value, index) {
                ...
                ctm_call_price +=parseFloat(value['ctm_price']);
              });
              ctm_call_id = no_of_records_array.toString();
            }
            calculatePrice();
          }else{
            calculatePrice();
      ...
      const calculatePrice = async () =>{
        if (msa_price.error) {
          ...
          const over_miles_price = parseFloat(
            pricing["per_mile_rate_over_ten_miles"].replace("$", "")
          );
          ...
          if (service_type === "Towing") {
            ...
            base_price = parseFloat(
              (
                parseFloat(pricing["retail_tow_rate"].replace("$", "")) +
                night_charges
              ).toFixed(2)
            );
              ...
              if (!err) {
                ...
                if (distances.rows[0]["elements"][0].status === "OK") {
                  ...
                  net_price = parseFloat(
                    (net_price + additional_charges + ctm_call_price).toFixed(2)
                  );
                  total_price = parseFloat(
                    (net_price + net_price * service_charges).toFixed(2)
                  );
            ...
            // Base rate calculation on location timestamp
            base_price =
              parseFloat(pricing["retail_light_service_rate"].replace("$", "")) +
              night_charges;
            net_price = base_price;
...
router.get("/getInvoiceNumber", authMiddleware, async (req, res) => {
  ...
  if (lastInvoice_Id == null) {
    newInvoiceId = await InvoiceModel.getNewInvoiceNumber(
      initialInvoiceId,
      userId
    );
  } else {
...
router.post(
  ...
  [
    authMiddleware,
    [
      check("invoicenumber", "Invalid invoice number")
        .not()
        .isEmpty(),
      ...
      check("phone", "Invalid phone number")
        .not()
        .isEmpty(),
      check("year", "Invalid car year")
        .not()
        .isEmpty(),
      check("make", "Invalid car make")
        .not()
        .isEmpty(),
      check("model", "Invalid car make")
        .not()
        .isEmpty(),
      check("color", "Invalid car color")
        .not()
        .isEmpty(),
      check("servicetype", "Invalid service type")
        .not()
        .isEmpty(),
      // check('problemtype', 'Invalid problem type').not().isEmpty(),
      check("anyonewithvehicle", "Invalid anyone with the vehicle data")
        .not()
        .isEmpty(),
      check("keysforvehicle", "Invalid keys for vehicle data")
        .not()
        .isEmpty(),
      ...
      check("pickuplocation", "Invalid pickup location")
        .not()
        .isEmpty(),
      // check('pickupnotes', 'Invalid pickup notes').not().isEmpty(),
      check("originaddress", "Origin address is missing")
        .not()
        .isEmpty(),
      ...
      check("baseprice", "Invalid base price")
        .not()
        .isEmpty(),
      check("additionalprice", "Invalid additional charges")
        .not()
        .isEmpty(),
      check("paymentamount", "Invalid payment amount")
        .not()
        .isEmpty(),
      check("paymenttotalamount", "Invalid payment total amount")
        .not()
        .isEmpty(),
      check("paymentemail", "Invalid payment email").isEmail(),
      check("sendpaymentto", "Invalid send payment link send type")
        .not()
        .isEmpty(),
      check("msa_system", "MSA system is required")
        .not()
        .isEmpty()
  ...
  async (req, res) => {
    ...
    if (response.error) {
      return res
        .status(500)
        .json({ errors: [{ msg: "Invalid invoice number" }] });
        ...
        if (draft === "1") {
          return res
            ...
            .json({
              errors: [],
              data: {
                msg: "Invoice details successfully saved",
          ...
          if (ctm_call_id !== "") {
            ...
            await InvoiceModel.ctmInvoiceUpdate(
              ctm_call_id,
              invoicenumber,
              payment_status
            );
            ...
            if (isSend) {
              ...
              return res
                ...
                .json({
                  ...
                  data: {
                    msg:
                      "Invoice details successfully saved. But SMS could not be sent to phone.",
                    invoice: invoice_number
                  }
                });
            }
            ...
            if (isSend) {
              ...
              return res
                ...
                .json({
                  ...
                  data: {
                    msg:
                      "Invoice details successfully saved. But email could not be sent.",
                    invoice: invoicenumber
                  }
                });
            }
      ...
      return res
        .status(500)
        .json({ errors: [{ msg: "Invoice Number does not exist!" }] });
    }
  }
);
...
router.post(
  ...
  [
    authMiddleware,
    [
      check("invoice_id", "Invalid invoice id")
        .not()
        .isEmpty(),
      ...
      check("last_name", "Invalid last name")
        .not()
        .isEmpty(),
      check("phone_number", "Invalid phone number")
        .not()
        .isEmpty(),
      check("year", "Invalid car year")
        .not()
  ...
  async (req, res) => {
    ...
    if (response.error) {
      ...
      if (invoice.error) {
        ...
        const invoiceData = await InvoiceModel.getInvoiceByInvoiceId(
          invoice_id
        );
        return res
          .status(200)
          .json({
            errors: [],
            data: {
              msg: "Invoice details successfully updated",
              invoice: invoiceData.result[0]
            }
          });
      }
    }
...
router.post(
  ...
  async (req, res) => {
    ...
    if (response.error) {
      ...
      const update = await InvoiceModel.updateLinkDate({
        invoice_id,
        send_payment_to
      });
      ...
      if (invoiceinfo.error) {
        ...
        const date_opened =
          moment(invoice.date_opened_timestamp).format(
            "ddd MMM D YYYY kk:mm:ss"
          ) +
          " GMT " +
          moment().format("Z");
        const date_first_link_send =
          moment(invoice.date_opened_timestamp).format(
            "ddd MMM D YYYY kk:mm:ss"
          ) +
          " GMT " +
          moment().format("Z");
        const data_last_link_send = invoice.date_edit_timestamp
          ? moment(invoice.date_edit_timestamp).format(
            "ddd MMM D YYYY kk:mm:ss"
          ) +
          " GMT " +
          moment().format("Z")
          : "";
        const data_payment_done = invoice.date_payment
          ? moment(invoice.date_payment).format("ddd MMM D YYYY kk:mm:ss") +
          " GMT " +
          moment().format("Z")
          : "";
        ...
        if (send_payment_to === "Email") {
          ...
          if (isSend) {
            return res
              ...
              .json({
                errors: [],
                data: { msg: "Payment link send again", invoice }
              });
          } else {
            return res
              .status(500)
              .json({
                errors: [],
                data: { msg: "Payment link could not be send again", invoice }
              });
          }
        } else {
          // Send SMS
        ...
        if (send_payment_to === "Email") {
          ...
          if (isSend) {
            return res
              ...
              .json({
                errors: [],
                data: { msg: "Payment receipt send again", invoice }
              });
          } else {
            return res
              .status(500)
              .json({
                errors: [],
                data: {
                  msg: "Payment receipt could not be send again",
                  invoice
                }
              });
          }
...
router.get("/:invoicenumber", authMiddleware, async (req, res) => {
  ...
  if (isResponse.error) {
    ...
    if (response.error) {
      ...
      return res
        .status(200)
        .json({ errors: [], data: { msg: "Invoice details", invoice } });
    }
  }
});
...
router.post("/", authMiddleware, async (req, res) => {
  ...
  if (dataArray.error) {
    ...
    let csv = "";
    try {
      csv = json2csv(dataArray.result.length > 0 ? dataArray.result : [], {
        csvFields
      }); // json2csv((dataArray.result.length > 0 ? dataArray.result : []), { csvFields }); // json2csv({data: dataArray.result, csvFields });
    } catch (json2csverror) {
