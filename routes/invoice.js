...
router.get('/send-mail', async (req, res) => {
	...
	const paymentUrl = req.body.paymentUrl
	const invoice_number = invoice_number;

	const receiver = payment_email;
	const mail_subject = 'Reg. Link to pay for your Service Request';
	var mail_message = '<div>';
	'<p>Hello' + first_name + ',</p>' +
		'<br>' +
		'<p><b>Invoice Number</b>: ' + first_name + '</p>' +
		'<br>' +
		'<p><b>You can pay for your Tow @ this link: <a href=" ' + paymentUrl + ' " target="_blank"> ' + paymentUrl + '</a></b></p>' +
		'<br>' +
		...
		shortUrl.short('http://ec2-18-217-104-6.us-east-2.compute.amazonaws.com/payment/' + invoice_number, function (err, paymentUrl) {
			...
			if (mode == 'add') {
				if (sendpaymentto == "phone") {
					...
					//sendEmail(paymentemail, fname, servicetype, paymentamount, paymentUrl, invoice_number);
...
// 	const payment_email = req.params.invoice_number;
// 	shortUrl.short('http://google.com/', function (err, url) {
// 		return url;
// 	});
// });
